...
class Table(BaseModel):
    ...
    def verify(self) -> Self:
        ...
        # 3. check if the col names is not illegal
    ...
    def extract_schema(self) -> TableSchema:
        return TableSchema(col_types=self.col_types)
    ...
    def fast_hash(self) -> str:
        ...
        col_types_hash = hashlib.md5(
            json.dumps({k: v.value for k, v in self.col_types.items()}, sort_keys=True).encode("utf-8")
        ).hexdigest()
    ...
    def to_view(self) -> TableView:
        ...
        for col in self.df.columns:
            ...
            if self.col_types[col] == ColType.DATETIME:
                cols[col] = [v.isoformat() if isinstance(v, datetime) else v for v in self.df[col].tolist()]
            else:
                normalized = []
...
class Model(BaseModel):
    ...
    def extract_schema(self) -> Schema:
        return Schema(type=Schema.Type.MODEL, model=self.metadata)
...
class Data(BaseModel):
    ...
    def extract_schema(self) -> Schema:
        if isinstance(self.payload, Table):
            return Schema(type=Schema.Type.TABLE, tab=self.payload.extract_schema())
    ...
    def to_view(self) -> "DataView":
        if isinstance(self.payload, Table):
            ...
            type_map = {str: "str", int: "int", float: "float", bool: "bool", File: "File"}
