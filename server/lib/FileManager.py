...
@celery_app.task(name="server.lib.FileManager.cleanup_guest_data_task")
def cleanup_guest_data_task() -> None:
    """
    Periodic task to clean up old data and files generated by the Guest user.
    """
    start_time = time.perf_counter()
    db_client = next(get_session())
    file_manager = FileManager(sync_db_session=db_client)
    ...
    try:
        ...
        guest_user = db_client.query(UserRecord).filter_by(username=GUEST_USER_USERNAME).first()
        if not guest_user:
            logger.info("Guest user not found, skipping cleanup.")
            return

        # 2. Define expiration (e.g., data older than 2 hours)
        expiration_time = datetime.now(timezone.utc) - timedelta(hours=2)

        # 3. Cleanup NodeOutputRecord and Temporary Projects
        # By deleting the temporary projects created for the playground, 
        # NodeOutputRecord will be cascade deleted due to the foreign key constraint.
        # temporary projects prefix is 'temp-playground-'
        temp_projects = db_client.query(ProjectRecord).filter(
            ProjectRecord.owner_id == guest_user.id,
            ProjectRecord.name.like("temp-playground-%"),
            ProjectRecord.updated_at < expiration_time
        ).all()

        if temp_projects:
            logger.info(f"Cleaning up {len(temp_projects)} expired guest temporary projects.")
            for proj in temp_projects:
                db_client.delete(proj)

        # 4. Cleanup Guest Files
        # Files are not cascade deleted by project deletion (project_id sets to NULL by trigger or constraint)
        # So we physically delete old guest files.
        guest_files = db_client.query(FileRecord).filter(
            FileRecord.user_id == guest_user.id,
            FileRecord.last_modify_time < expiration_time
        ).all()

        if guest_files:
            logger.info(f"Cleaning up {len(guest_files)} expired guest files.")
            for file_rec in guest_files:
                try:
                    file_manager.minio_client.remove_object(
                        bucket_name=file_manager.bucket, 
                        object_name=str(file_rec.file_key)
                    )
                    db_client.delete(file_rec)
                except Exception as e:
                    logger.warning(f"Failed to delete guest file {file_rec.file_key}: {e}")

        # 5. Execute DB updates and Trigger Cascade Delete
        db_client.commit()
    except Exception as e:
        logger.exception(f"Error during guest data cleanup: {e}")
        db_client.rollback()
    finally:
        db_client.close()
        logger.info(f"Guest cleanup finished in {time.perf_counter() - start_time:.2f} seconds.")
