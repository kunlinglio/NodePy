...
async def login(
    ...
    try:
        ...
        result = None
        if req.type == "email":
            result = await db_client.execute(select(UserRecord).where(UserRecord.email == req.identifier))
        elif req.type == "username":  # username
            result = await db_client.execute(select(UserRecord).where(UserRecord.username == req.identifier))
        else:
            assert False, "Unreachable"
        user = result.first()

        if user is None or not AuthUtils.verify_password(req.password, user[0].hashed_password):
            raise HTTPException(
                status_code=401,
        ...
        if user[0].username != ADMIN_USER_USERNAME:
            raise HTTPException(status_code=401, detail="Only admin user can login")
...
async def refresh_access_token(
    ...
    try:
        ...
        if user is None:
            ...
            raise HTTPException(status_code=401, detail="Only admin user can refresh access token by this method")
